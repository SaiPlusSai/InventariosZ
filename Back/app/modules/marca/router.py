...
from app.modules.marca.schemas import PreviaImportacionResponse, ConfirmarImportacionRequest
...
def exportar_excel(db: Session = Depends(get_db)):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=Marca_inventario.xlsx"}
    )
...
def importar_plantilla():
    ...
    return StreamingResponse(
        buffer,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename=plantilla_Marca.xlsx"}
    )
