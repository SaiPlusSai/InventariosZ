...
const ImportarModal = ({ onClose, onImportSuccess }) => {
  ...
  const handleDescargarPlantilla = async () => {
    try {
      ...
      const response = await productoService.descargarPlantilla();
  ...
  const handleFileChange = async (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Upload para vista previa
      setLoading(true)
      try {
        const formData = new FormData()
        formData.append('file', selectedFile)
        const response = await productoService.importarPrevia(formData)
  ...
  const handleConfirmar = async () => {
    ...
    setLoading(true)
    try {
      const res = await productoService.importarConfirmar({ filas: previaData.filas })
      toast.success(`Se importaron ${res.data.creados} productos correctamente`)
      onImportSuccess()
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Importación Masiva</h2>
            <p className="text-sm text-gray-500">Añade múltiples productos usando Excel</p>
          </div>
                    ...
                    <tbody>
                      {previaData.filas.map((f, i) => (
                        <tr key={i} className="border-b last:border-0 hover:bg-gray-50">
                          ...
                          <td className="px-4 py-2 text-gray-900 font-medium">{f.codigo || '-'}</td>
